...
- **A missing `complianceStatus` is never a failing one.** `pipelineExists === false` is a
  ...
  for. The same rule is why the CI filter's `no-pipeline` reads `=== false`.
